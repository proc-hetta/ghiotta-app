...
@freezed
abstract class MoveToLevelWithOnOffPayload with _$MoveToLevelWithOnOffPayload {
  const MoveToLevelWithOnOffPayload._();
  const factory MoveToLevelWithOnOffPayload({required int level}) =
      _MoveToLevelWithOnOffPayload;

  factory MoveToLevelWithOnOffPayload.fromJson(Map<String, Object?> json) =>
      _$MoveToLevelWithOnOffPayloadFromJson(json);
}
...
abstract class ApiClient {
  ...
  @GET("/api/v1/smart-devices/{nodeId}")
  Future<SmartDevice> getSmartDevice(@Path() int nodeId);

  @GET("/api/v1/admin/authorized-devices")
  Future<List<Device>> getDevices();

  @POST("/api/v1/admin/authorized-devices")
  Future<Device> createDevice(@Body() NewDevice newDevice);

  @POST("/api/v1/smart-devices")
  Future<SmartDevice> commissionSmartDevice(
    @Body() SmartDeviceCommission smartDeviceCommission,
  );

  @POST("/api/v1/smart-devices/{nodeId}/commands/toggle")
  Future<void> toggle(@Path() int nodeId);

  @POST("/api/v1/smart-devices/{nodeId}/commands/moveToLevelWithOnOff")
  Future<void> moveToLevelWithOnOff(
    @Path() int nodeId,
    @Body() MoveToLevelWithOnOffPayload payload,
  );

  @DELETE("/api/v1/smart-devices/{nodeId}")
  Future<void> deleteSmartDevice(@Path() int nodeId);

  @DELETE("/api/v1/admin/authorized-devices/{id}")
  Future<void> deleteDevice(@Path() int id);
}
