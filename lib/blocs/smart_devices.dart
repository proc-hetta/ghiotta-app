...
@freezed
abstract class SmartDevicesCreate extends SmartDevicesEvent
    with _$SmartDevicesCreate {
  const SmartDevicesCreate._();
  const factory SmartDevicesCreate({
    required SmartDeviceCommission smartDeviceCommission,
  }) = _SmartDevicesCreate;
}

@freezed
abstract class SmartDevicesDelete extends SmartDevicesEvent
    with _$SmartDevicesDelete {
  const SmartDevicesDelete._();
  const factory SmartDevicesDelete({required int nodeId}) = _SmartDevicesDelete;
}
...
class SmartDevicesBloc extends Bloc<SmartDevicesEvent, SmartDevicesState>
  ...
  SmartDevicesBloc({required this.talker, required this.api})
    ...
    on<SmartDevicesCreate>(_onSmartDevicesCreate, transformer: droppable());
    on<SmartDevicesDelete>(_onSmartDevicesDelete, transformer: droppable());
  ...
  Future<void> _onSmartDevicesCreate(
    SmartDevicesCreate event,
    Emitter<SmartDevicesState> emit,
  ) async {
    try {
      await api.client.commissionSmartDevice(event.smartDeviceCommission);
  ...
  Future<void> _onSmartDevicesDelete(
    SmartDevicesDelete event,
    Emitter<SmartDevicesState> emit,
  ) async {
    try {
      await api.client.deleteSmartDevice(event.nodeId);
    } on DioException catch (s, t) {
      talker.handle(s, t);
      emitPresentation("Network error: ${s.response?.statusCode ?? "Unknown"}");
      return;
